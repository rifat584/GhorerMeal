...
const Card = ({ meal }) => {
  ...
  return (
   <article className="group cursor-pointer overflow-hidden rounded-[1.75rem] border border-base-300 bg-base-100 p-4 shadow-sm transition hover:-translate-y-1 hover:shadow-lg flex flex-col h-full">
  <div className="flex flex-col gap-4 flex-1"> 
    ...
    <div className="space-y-2 flex-1"> 
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-lg font-semibold text-base-content line-clamp-2">{foodName}</h3>
    ...
    <Link to={`/meal/${_id}`} className="btn btn-primary w-full rounded-full mt-auto">
      See Details
    </Link>
  </div>
</article>
